    ...
    try:
        ...
        parsed_data = {"task":request_json["task"],
                    "deadline":request_json["deadline"],
                        ...
                        "lastUpdated":now}
        Todo_list_model(**parsed_data)  # validate if the request match the todo_list model
        insert_response = todo_collection.replace_one({"_id":ObjectId(id)},request_json) # Object id cant be casted to json
    except:
        abort(400)
    request_json.update({"_id":id,"lastUpdated":now})
...
@todo_bp.route('/todo_list/deleted/<id>',methods=["GET"])
def get_soft_deleted(id):
    try:
        response = soft_deleted_todo_list.find_one({"_id":ObjectId(id)})
        response.update({"_id":id})
    except:
        abort(404)
    return jsonify(response)
